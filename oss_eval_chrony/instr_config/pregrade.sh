...
CONF_FILE="/home/student/chrony.conf"

# Kiểm tra điều kiện đầu tiên
if grep -q "local stratum" "$CONF_FILE"; then
    stratum_check="TRUE"
else
    stratum_check="FALSE"
fi
...
if grep -q "allow "$CONF_FILE"; then
    allow_check="TRUE"
else
    allow_check="FALSE"
fi
...
if [[ "$stratum_check" == "TRUE" && "$allow_check" == "TRUE" ]]; then
    echo "Both conditions are satisfied." > /home/student/.local/results/chrony_check.result
else
    echo "One or both conditions are missing." > /home/student/.local/results/chrony_check.result
fi
